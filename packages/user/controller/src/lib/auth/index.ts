...
export const social = (c: Context) => {
  ...
  if (!SOCIAL_PROVIDERS.includes(social as any)) {
    return c.text(
      `Unsupported provider: ${social}. Use: ${SOCIAL_PROVIDERS.join(', ')}`,
      400,
    );
  }
...
export const socialCallback = async (c: Context) => {
  ...
  c.header(
    'Set-Cookie',
    `token=${token}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=${ENV.TWENTY_FOUR_HOURS * 2}`,
  );
