...
export const getway = async (entry: ITransaction, user: IUser, envObj?: Record<string, any>) => {
  const FRONTEND_URL = String(getEnv(envObj, ENV.FRONTEND_URL, DEFAULTS.FRONTEND_URL));
  const BACKEND_URL = String(getEnv(envObj, ENV.BACKEND_URL, DEFAULTS.BACKEND_URL));
  const ACCESS_TOKEN = String(getEnv(envObj, ENV.MERCADO_PAGO_ACCESS_TOKEN, DEFAULTS.MERCADO_PAGO_ACCESS_TOKEN));
...
export const webhook = async (c: Context) => {
  ...
  const ACCESS_TOKEN = String(
    getEnv(c.env, ENV.MERCADO_PAGO_ACCESS_TOKEN, DEFAULTS.MERCADO_PAGO_ACCESS_TOKEN)
  );
