...
export const getAll = async (c: Context) => {
  ...
  const result = await (c.get('testingService') as TestingService).listActive({ page, limit });
  return c.json(result);
};

export const getByDomain = async (c: Context) => {
  const domain = c.req.param('domain') ?? '';
  const result = await (c.get('testingService') as TestingService).listByStore(domain);
  return c.json(result);
};

export const getByCampaing = async (c: Context) => {
  const campaing = c.req.param('campaing') ?? '';
  const result = await (c.get('testingService') as TestingService).getByCampaing(campaing);
...
export const getUser = async (c: Context) => {
  ...
  const result = await (c.get('testingService') as TestingService).listByUser(c.get('user').id, { page, limit });
  return c.json(result);
};
