...
export const testing = sqliteTable(
  'testing',
  {
    ...
    type: text('type', {
      enum: ['TIME', 'VIEWS', 'CART', 'PURCHASES'],
    }).notNull(),
  ...
  (table) => [
    ...
    index('user_campaing_idx').on(
      (table as any).userId,
      (table as any).campaing,
    ),
    index('country_active_campaing_idx').on(
      (table as any).countryCode,
      (table as any).isActive,
      (table as any).campaing,
    ),
  ],
);
...
export const testingStores = sqliteTable(
  'testing_stores',
  {
    testingId: text('testing_id')
      .notNull()
      .references(() => ({ table: 'testing', column: 'id' }) as any),
    storeId: text('store_id')
      .notNull()
      .references(() => ({ table: 'stores', column: 'id' }) as any),
  },
  (table) => [
